...
import "@/components/case-study/case-study-zone-keyed-video.css";
...
const DEFAULT_KEY_RGB = { r: 0, g: 219, b: 251 } as const;
const KEY_THRESHOLD = 72;

export type ZoneKeyedCrop = {
  sx: number;
  sy: number;
  sw: number;
  sh: number;
...
export type ZoneKeyedRgb = {
  r: number;
  g: number;
  b: number;
};

type CaseStudyZoneKeyedVideoProps = {
  webmSrc: string;
  sourceSrc: string;
  crop: ZoneKeyedCrop;
  label: string;
  className?: string;
  /** Override when a clip uses a different chroma plate color. */
  keyRgb?: ZoneKeyedRgb;
};
...
export function CaseStudyZoneKeyedVideo({
  webmSrc,
  sourceSrc,
  crop,
  label,
  className,
  keyRgb = DEFAULT_KEY_RGB,
}: CaseStudyZoneKeyedVideoProps) {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      if (video.readyState >= 2) {
        ...
        chromakeyFrame(ctx, width, height, keyRgb);
      }
  ...
  return (
    ...
    >
      ...
      <div className="case-study-zone-keyed__controls">
        <button
          type="button"
          className="case-study-zone-keyed__control"
